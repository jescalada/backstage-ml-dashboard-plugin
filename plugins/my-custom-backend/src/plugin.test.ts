...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [myCustomPlugin],
    });

    await request(server).get('/api/my-custom/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/my-custom/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/my-custom/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/my-custom/todos/${createdTodoItem.id}`)
